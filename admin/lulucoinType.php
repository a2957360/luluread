<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['submit'])){
        ...
        $stmt = $pdo->prepare("INSERT into `lulucoinType` (`Name`,`Price`,`Bonus`) VALUES ('$Name','$Price','$Bonus')");
        $stmt->execute();
        if($stmt != null){
    ...
    if(isset($_POST['delsubmit'])){
        $typeId=$_POST['typeId'];
        $stmt = $pdo->prepare("DELETE FROM `lulucoinType` WHERE `Id` = '$typeId'");
        $stmt->execute();
        if($stmt != null){
        	header('location: '.$_SERVER['HTTP_REFERER']);
          }else{
    ...
    if(isset($_POST['changesubmit'])){
        ...
        $stmt = $pdo->prepare("UPDATE `lulucoinType` SET `Name` = '$Name',`Price` = '$Price',`Bonus` = '$Bonus'  WHERE `Id`='$typeId'");
        $stmt->execute();
        if($stmt != null){
        	header('location: '.$_SERVER['HTTP_REFERER']);
        }else{
  	...
  	$typeshowlist= array();
	$stmt = $pdo->prepare("SELECT * FROM `lulucoinType`");
	$stmt->execute();
	if($stmt != null){
                        ...
                        <div class="col-md-12">
                            <div class="card  card-tasks">
                                <div class="card-header ">
                                    <h4 class="card-title">添加打赏类别</h4>
                                </div>
                                <div class="card-body ">
                                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" enctype='multipart/form-data'>
                                        CAD：<input type="text" name="Name">
                                        LULUCoin：<input type="text" name="Price">
...
<script src="assets/js/demo.js"></script>
        <script type="text/javascript">
          ...
          $(document).ready(function() { 
              $("#lulucoinType").attr("class","nav-item active");
          }); 
