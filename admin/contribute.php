<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['submit'])){
        ...
        $stmt = $pdo->prepare("INSERT into `contributeType` (`Name`,`Price`,`Bonus`) VALUES ('$Name','$Price','$Bonus')");
        $stmt->execute();
        if($stmt != null){
    ...
    if(isset($_POST['delsubmit'])){
        $typeId=$_POST['typeId'];
        $stmt = $pdo->prepare("DELETE FROM `contributeType` WHERE `Id` = '$typeId'");
        $stmt->execute();
        if($stmt != null){
        	header('location: '.$_SERVER['HTTP_REFERER']);
          }else{
    ...
    if(isset($_POST['changesubmit'])){
        ...
        $stmt = $pdo->prepare("UPDATE `contributeType` SET `Name` = '$Name',`Price` = '$Price',`Bonus` = '$Bonus'  WHERE `Id`='$typeId'");
        $stmt->execute();
        if($stmt != null){
        	header('location: '.$_SERVER['HTTP_REFERER']);
        }else{
...
<script src="assets/js/demo.js"></script>
        <script type="text/javascript">
          ...
          $(document).ready(function() { 
              $("#contribute").attr("class","nav-item active");
          }); 
